...
optim.obj <- function(par1,exp.depth=depth, y, weight=-0.0001){

  R <-dbeta(y, shape1=par1, shape2=exp.depth-par1, log=T) #log should be F- because if Log=T we could have positive and negative values. We should add log an(R))
  v <- sd(y)
  range<- max(y)-min(y)
...
fit.variants.weight <- function(variants){   #to calculate the initial matrix
  ...
  fit1<-optimize(y=variants,weight=weight,exp.depth=depth ,f=optim.obj, interval = c(1,199),tol=0.001)#,lower=1, upper=200, control=list(factr=1e-3, maxit=200, trace=0)) #, lower=1.5, upper=1000, method='L-BFGS-B'
...
#'
#' @param arg.data Dataframe of the samples. Each sample should be in a separate column.
#'
...
#' @param sampleNum This parameter corresponds to the column of the dataframe that the sample is stored in.
#'
...
#'
#' @export
mag.single <- function(arg.data, sampleNum=1){
...
#' @param shape Shape corresponds to the variance of the germline sample variance. In most experiments we take shape to be around 8 or 10.
#' @param depth Depth is the sequence coverage of the experiment. By default the algorithm uses depth of 200.
#'
#' @return A list of R files. The main output is final.data which has the samples and the cluster assingment in a dataframe.
#'
#' @export
find.cut.off <- function(mag.output, shape, depth){
  time0 <- as.numeric(Sys.time())
...
#'
#' @export
mag.reduce.graph<- function(arg.data,x.el=data.frame() ,sampleNum=1, n=-100){
  ...
  if(nn < size.threshold | nn == n) {
    ...
    mag.reduce.graph(arg.data=data, sampleNum=sampleNum,x.el=x.el.final,n=nn)
  }
...
#' @param sampleNum This parameter corresponds to the column of the dataframe that the main sample is stored in.
#'
...
#' @export
mag.multiple.cut <- function(arg.data,x.el.cut=data.frame(), sampleNum=1) {

  if(sampleNum>dim(arg.data)[2]-1){print("stop the algorithm and check the sampleNum; run mag.prepdata() first")}
...
find.cut.off.single <- function(var.mean, data, x.el.list, shape, depth){
  ...
  while(flag == FALSE){
    ...
    if(sum(ok.points & stepI.x.el.step) == 0){     ###meaning stepI.x.el is not in ok.points
      ...
      #print("stepII breaks check")
      #print(stepII.breaks)
      #print(dim(stepII.breaks))
      #print("stepII x.el")
      #print(stepII.breaks.x.el)
      ...
      for(ii in 1:2){
        ...
        # m<-c(m,mean(var.sim.temp))
        #print("m")
        #print(m)
        #print("vv")
        #print(vv)
        #print(newtemp[ii,2])
        #print(m-newtemp[ii,2])
        #print(vmax-stepII.breaks[ii,2])
        #print(paste("II", ii))
        #print(ii)
...
#' @export
find.cut.off.multiple <- function(mag.var, shape, depth){

  if(is.null(mag.var[['var.mean.all']])) return('incomplte, run mag.var() first');

  final.data.clusters<- mag.var$mag.out$data.sorted
  cuts<-list()
  colors.all<-c()
  ...
  for(i in 1:length(mag.var[['var.mean.all']])) {
    ...
    cut.temp<-find.cut.off.single(var.mean = var.mean, data = data,x.el.list = x.el, shape, depth )
    cuts[[i]]<-cut.temp
...
#'
#' @export
mag.var<- function(mag.out, sampleNum=1){

  #varmean is geneated based on sorted data
  x.el.list<- mag.out$x.el
  data<- data.frame(mag.out$data.sorted)
